...
| 姓名匹配员工号 | ✅ | 识别姓名并匹配员工号。 |
| 锁班皇帝 | ✅ | Playwright 模拟锁班，速度较慢的批量录入工具。 |
| HTTP 锁班皇帝（新） | 🧪 | 好活，使用 HTTP 直接 POST 锁班信息。 |
| 飞行经历查询 | ✅ | 批量查询飞行时间+起落数、飞行经历+起落数、左座经历+起落数或全部数据。 |
| HTTP 飞行经历查询（新） | 🧪 | 复用已登录会话，通过纯 HTTP 串行查询完整飞行经历和起落数。 |
