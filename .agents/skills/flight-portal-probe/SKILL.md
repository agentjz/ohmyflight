...
description: IEB/飞行门户专用 Playwright 探测和批量自动化流程，覆盖人工扫码登录、资质管理/飞行训练/技术资料/资料管理入口、员工号逐人查询、个人技术等级和运行资格表格、非生产任务锁班录入与查询、休假额度识别、冲突锁班精准解锁重提、跨 frame 与 rowspan/colspan 解析、样例核对、Excel 逐人即时写回、中断恢复和证据留存。用户提到 https://ieb.csair.com、飞行门户、员工号查询、技术等级、运行资格、区域航线资格、单飞资质、锁班、健康疗养、公休假、日期冲突、解锁、录制脚本稳定化或相关 Python 自动化时使用。
---
...
入口为“运行管理 -> 非生产任务”。录入使用“非生产任务录入”，历史记录和解锁使用“锁班查询”。页面可能同时保留两个标签页 DOM，`#lockType` 等重复 id 必须限定在当前容器内：录入类型使用 `#showNonproductionTaskImportPage #lockType`，查询类型使用 `#showNonproductionTaskLockPage1 #lockType`。

录入时先输入员工号并确认隐藏员工号和姓名，再选择锁班类型。只有员工和类型就绪后，页面底部的休假限制表才可作为事实读取。表头通常在 `.hDiv table`，数据在 `.bDiv table`；按“休假类型、年份、休假天数、锁班天数、解锁天数、可休天数”映射，不能按列号。`RECU_LVE-健康疗养` 与 `ALV_FD-飞行员公休（订座）` 可按目标年份“可休天数”路由；额度表里的 ALV_FD 可能显示为“年假（公休假）”，不要据此改成普通 `ALV`。其他锁班类型原样提交，不读取这两类额度。

提交结果分别读取结果表和冲突表，并按员工号、姓名、实际类型和起止日期归属当前记录。发生冲突时按以下顺序处理：

1. 点击结果页“锁班查询”，输入当前员工号并等待隐藏员工号一致；筛选“已锁”，逐页读取全部结果。
2. 保留空白选择列表头，其他字段按表头映射；部门、锁班原因、冲突等被省略的单元格优先读取 `title` 完整值。
3. 只以“员工号一致 + 状态为已锁 + 旧记录自然日区间与冲突区间重叠”筛选。必须全分页恰好一条；零条或多条都记录失败并跳过当前输入，不解锁。
4. 不依赖 checkbox 的 `value`、序号、录制时 accessible name 或固定行号。唯一候选确认后，在该行 DOM 内找最前面的 `.lockTaskListIds` checkbox；checkbox 只是执行入口，不参与身份判断。
5. 解锁的是门户旧整行，不是只解除相交日期。执行前把旧行完整字段落盘；结果文件无法保存时不得解锁。
6. 点击“解锁”后有两层确认。第一层“原因”是 jQuery UI 弹窗，`#writeRemark` 会被移到 `body`，直接等待全页可见 `#remark`，沿祖先定位 `.ui-dialog` 并点击“确定”。随后读取结果弹窗，只有明确包含“成功”才点击第二个“确定”并继续。
7. 重新查询并确认旧行不再属于“已锁”结果，再回录入页重提原请求一次。重提仍冲突时记录并跳过当前输入，不继续解锁第二条，不中断后续输入。

不要把旧锁班摘要写进新锁班的“锁班原因”；新锁班沿用用户统一备注或门户默认原因。结果 Excel 的“备注”写“已解锁”以及旧记录的锁班名称、锁班原因、开始日期、结束日期；序号、类型、录入人等需要审计的字段放独立列。
