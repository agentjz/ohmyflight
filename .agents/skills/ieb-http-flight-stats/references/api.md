...
- 4 个独立 Session 并发查询 4 人：4 条均为 HTTP 200、每条唯一匹配当前员工、19 列一致。
- 新 APP 使用 4-worker 查询 44 条真实模板：有效 44、成功 44、失败 0，总耗时约 14 秒。
- 最终原版为 44 行、门户字段 19 列；原版/去分钟版形状一致，三类经历分钟转换核对通过。

上述耗时只说明当次会话和网络事实，不写成永久性能承诺。更高并发数尚未验证，不作为默认值。
