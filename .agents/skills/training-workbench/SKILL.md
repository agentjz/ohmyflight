...
- `smart-schedule.ts`：复用当前轮次与规则生成跨年滚动合法月份，组织原始到期压力、人工方案、全局均衡方案、CRM 固定底座和项目展示投影；未来人工安排与模拟排班只进入人工方案，不作为智能推荐输入。
- `smart-schedule-optimizer.ts`：把同项目、同人天、同安全目标和同合法月份的任务分组；连续线性规划按总峰值、安全提前目标、动态避让月份、总平均偏差和项目峰值分阶段求解并锁定前序最优值，再整数化并按同顺序做合法单任务移动，末级改善项目内部偏差；不承载有效期或页面规则。
