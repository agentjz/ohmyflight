...
- `smart-schedule.ts`：复用当前轮次与规则生成合法月份，组织一套全年全项目智能方案和项目展示投影；未来人工安排不作为推荐月份偏好。
- `smart-schedule-optimizer.ts`：把同项目、同人天和同合法月份的任务分组；连续线性规划按全年总峰值、全年平均偏差、各项目峰值、项目内部平均偏差分阶段求解并锁定前序最优值，再整数化并做同顺序的合法单任务移动；不承载有效期或页面规则。
