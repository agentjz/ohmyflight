...
用户从 ohmyflight 工具主页点击“技能”，在新标签页打开 Jobskill。页面与工具首页、开发者页和用户手册页共用柔和野兽派双主题壳层，默认使用白天模式并支持在顶栏切换主题；搜索框、目录和正文使用方角完整边框、低饱和多色点缀与无模糊硬阴影，页面和正文滚动区域提供可见的细滚动条。桌面端通过左侧导航切换 Skill，移动端通过命令栏中的折叠菜单切换；右侧显示对应 Markdown 正文、表格和图片。
