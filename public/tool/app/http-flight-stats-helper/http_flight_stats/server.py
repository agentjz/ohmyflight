...
def payload_from_json(payload: dict[str, object]) -> InputPayload:
    ...
    raw_scope = payload.get("scope", "all")
    if isinstance(raw_scope, list):
        scope: str | list[str] = [str(value) for value in raw_scope]
    else:
        scope = str(raw_scope or "all")
    return InputPayload(
...
def create_server(
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "HttpFlightStats/1.0"
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if path.startswith("/api/download/"):
                ...
                self.send_header(
                    "Content-Disposition",
                    f"attachment; filename*=UTF-8''{quote(result_path.name)}",
                )
                self.send_header("Content-Length", str(len(content)))
                self.end_headers()
                self.wfile.write(content)
