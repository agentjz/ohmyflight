...
DATE_PATTERN = re.compile(r"(?P<year>\d{4})\s*(?:年|[-/.])\s*(?P<month>\d{1,2})\s*(?:月|[-/.])\s*(?P<day>\d{1,2})\s*(?:日)?")
...
def _record_from_values(
    ...
    start_date = normalize_date(start_value, today)
    if not start_date:
        raise ValueError("开始日期格式无效")
    end_date = normalize_date(end_value, today) if end_value not in (None, "") else start_date
    if not end_date:
...
def parse_pasted_records(text: str, today: date | None = None) -> tuple[list[QueryRecord], list[str]]:
    ...
    for index, line in enumerate(_split_pasted_lines(text), start=1):
        ...
        dates = [normalize_date(match.group(0), current_day) for match in DATE_PATTERN.finditer(line)]
        dates = [item for item in dates if item]
        if not dates:
            errors.append(f"第{index}条：未识别日期")
            continue
        start_value = dates[0]
        end_value = dates[1] if len(dates) > 1 else start_value
        try:
            record = _record_from_values(employee_id, name, start_value, end_value, f"粘贴第{index}条", current_day)
...
def read_excel_records(path: str | Path, today: date | None = None) -> tuple[list[QueryRecord], list[str]]:
    current_day = today or date.today()
    records: list[QueryRecord] = []
    errors: list[str] = []
    seen: set[str] = set()
    ...
    try:
        ...
        for row_number, row in enumerate(rows[1:], start=2):
            if not row or not any(value not in (None, "") for value in row):
                continue
            get = lambda key: row[indexes[key]] if indexes[key] < len(row) else None
            try:
