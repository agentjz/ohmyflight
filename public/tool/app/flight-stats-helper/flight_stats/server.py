...
from .exporter import ScopeValue, normalize_scope
from .runner import RunConfig, read_input, run_worker_process
...
class RunManager:
    ...
    def _apply_event(self, event: dict[str, Any]) -> None:
        ...
        if event_type == "status":
            ...
            kind = event.get("kind", "")
            if kind in self._state["checks"]:
                self._state["checks"][kind] = {
            ...
            self._state["resultPaths"] = {
                "original": event.get("original", ""),
                "stripped": event.get("stripped", ""),
            }
        elif event_type == "completed":
            self._state["phase"] = "completed"
            self._state["message"] = event.get("message", "查询完成")
            ...
            self._state["message"] = event.get("message", "查询失败")
    ...
    def prepare(
        ...
        with self._lock:
                ...
                self._state.update(
                    {
                        ...
                        "message": "已准备新一批数据，浏览器保持打开" if not auto_run else "已发送新一批查询",
                        "runId": config.run_id,
                    }
                )
            ...
            self._process = context.Process(
                ...
                name=f"flight-stats-{config.run_id}",
            )
            self._config = config
            self._state = self._idle_state()
            self._state.update(
                {"phase": "starting", "message": "正在启动查询浏览器", "runId": config.run_id}
            )
            self._process.start()
            return self.snapshot()
    ...
    def start(
        ...
        """保留 agent 使用的全链路启动入口。"""
        return self.prepare(input_mode, scope, pasted_text, excel_name, excel_bytes, auto_run=True)
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            state["canStart"] = state["canPrepare"]
            state["canReuseBrowser"] = can_reuse_browser
            state["canCheckData"] = process_alive and phase in {
                "prepared",
                "data_checked",
            }
