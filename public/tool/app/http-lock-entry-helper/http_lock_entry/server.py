"""Loopback HTTP service for the HTTP lock-entry workbench."""
...
def create_server(
    ...
    class Handler(BaseHTTPRequestHandler):
        server_version = "HttpLockEntry/1.0"
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if path == "/api/download/result":
                ...
                self.send_header("Content-Disposition", 'attachment; filename="http-lock-result.xlsx"')
                self.send_header("Content-Length", str(len(content)))
                self.end_headers()
                self.wfile.write(content)
                return
            static_map = {
                ...
                "/libs/bootstrap.min.css": workbench_asset(
