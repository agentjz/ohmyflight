...
from .models import InputIssue, QueryRecord
...
def read_excel_records(path: str | Path) -> tuple[list[QueryRecord], list[InputIssue]]:
    ...
    try:
        ...
        for row_number, row in enumerate(rows[1:], start=2):
            ...
            source = f"Excel 第{row_number}行"
            if not employee_id:
                issues.append(InputIssue(row_number, normalize_text(raw_employee), name, "员工号不是六位数字", source))
                continue
            if employee_id in seen:
                issues.append(InputIssue(row_number, employee_id, name, "重复员工号，已跳过", source))
                continue
            seen.add(employee_id)
            records.append(QueryRecord(row_number, employee_id, name, source))
    finally:
        workbook.close()
...
def read_input(input_mode: str, input_path: str = "", pasted_text: str = "") -> tuple[list[QueryRecord], list[InputIssue]]:
    if input_mode == "excel":
        if not input_path:
            raise ValueError("请选择 Excel 文件")
        return read_excel_records(input_path)
    if input_mode == "paste":
        if not str(pasted_text or "").strip():
            raise ValueError("请粘贴查询人员")
        return parse_pasted_records(pasted_text)
    raise ValueError(f"不支持的输入方式：{input_mode}")
