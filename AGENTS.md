...
- 涉及 SOC 运行信息网的随机置位申请、`KkTvlApply`、动态航班号、人员校验、Cookie/cURL 或 HTTP 提交时，看 `.agents/skills/soc-positioning/SKILL.md`；SOC 与 IEB 不得混用 skill 或接口事实。
...
- 提交信息必须使用简体中文。
- 提交信息写成一个不分段、不换行的完整段落，可以包含多句话；必须具体说明本次提交实际修改了什么和带来的行为变化，不用一句话强行概括。
- 提交前确认暂存区只包含本次任务相关文件。
